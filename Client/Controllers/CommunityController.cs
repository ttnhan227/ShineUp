                    ...
                    try
                    {
                        ...
                        if (postsJson.ValueKind == JsonValueKind.Array)
                        {
                            ...
                            foreach (var postElement in postsJson.EnumerateArray())
                            {
                                ...
                                try
                                {
                                    if (postElement.TryGetProperty("postID", out var postIdProp) &&
                                        postIdProp.ValueKind == JsonValueKind.Number)
                                    {
                                    ...
                                    {
                                        _logger.LogWarning(
                                            $"Post at index {postIndex} is missing or has invalid postID");
                                    }

                                    if (postElement.TryGetProperty("title", out var titleProp) &&
                                        titleProp.ValueKind == JsonValueKind.String)
                                    {
                                        post.Title = titleProp.GetString() ?? string.Empty;
                                    }

                                    if (postElement.TryGetProperty("content", out var contentProp) &&
                                    ...
                                    else
                                    {
                                        ...
                                        _logger.LogWarning(
                                            $"Post {post.PostID} has invalid or missing createdAt, defaulting to now");
                                    }

                                    if (postElement.TryGetProperty("userID", out var userIdProp) &&
                                        userIdProp.ValueKind == JsonValueKind.Number)
                                    {
                                        post.UserID = userIdProp.GetInt32();
                                    }

                                    if (postElement.TryGetProperty("username", out var usernameProp) &&
                                        usernameProp.ValueKind == JsonValueKind.String)
                                    {
                                        post.Username = usernameProp.GetString() ?? string.Empty;
                                    }

                                    if (postElement.TryGetProperty("fullName", out var fullNameProp) &&
                                        fullNameProp.ValueKind == JsonValueKind.String)
                                    {
                                        post.FullName = fullNameProp.GetString() ?? string.Empty;
                                    }

                                    if (postElement.TryGetProperty("profileImageURL", out var imgUrlProp) &&
                                        imgUrlProp.ValueKind == JsonValueKind.String)
                                    {
                                        post.ProfileImageURL = imgUrlProp.GetString() ?? string.Empty;
                                    }

                                    if (postElement.TryGetProperty("categoryName", out var categoryNameProp) &&
                                        categoryNameProp.ValueKind == JsonValueKind.String)
                                    {
                                        post.CategoryName = categoryNameProp.GetString();
                                    }

                                    if (postElement.TryGetProperty("likesCount", out var likesCountProp) &&
                                        likesCountProp.ValueKind == JsonValueKind.Number)
                                    {
                                        post.LikesCount = likesCountProp.GetInt32();
                                    }

                                    if (postElement.TryGetProperty("commentsCount", out var commentsCountProp) &&
                                        commentsCountProp.ValueKind == JsonValueKind.Number)
                                    {
                                        post.CommentsCount = commentsCountProp.GetInt32();
                                    }

                                    if (postElement.TryGetProperty("hasLiked", out var hasLikedProp) &&
                                        hasLikedProp.ValueKind == JsonValueKind.True)
                                    {
                                        post.HasLiked = true;
                                    }

                                    // Handle media files if present
                                    if (postElement.TryGetProperty("mediaFiles", out var mediaFilesProp) &&
                                        mediaFilesProp.ValueKind == JsonValueKind.Array)
                                    {
                                        ...
                                        foreach (var mediaElement in mediaFilesProp.EnumerateArray())
                                        {
                                            ...
                                            if (mediaElement.TryGetProperty("url", out var mediaUrlProp) &&
                                                mediaUrlProp.ValueKind == JsonValueKind.String)
                                            {
                                                media.Url = mediaUrlProp.GetString() ?? string.Empty;
                                            }

                                            if (mediaElement.TryGetProperty("type", out var mediaTypeProp) &&
                                                mediaTypeProp.ValueKind == JsonValueKind.String)
                                            {
                                                media.Type = mediaTypeProp.GetString()?.ToLower() ?? "image";
                                            }

                                            if (mediaElement.TryGetProperty("publicId", out var publicIdProp) &&
                                                publicIdProp.ValueKind == JsonValueKind.String)
                                            {
                            ...
                            _logger.LogInformation(
                                $"Successfully processed {posts.Count} posts. Assigning to view model...");
