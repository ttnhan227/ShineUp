...
"
    ...
    private string DetectLanguage(string input)
    {
        ...
        if (Regex.IsMatch(input, @"[\u0400-\u04FF]"))
        {
            return "ru"; // Cyrillic
        }

        if (Regex.IsMatch(input, @"[\u0600-\u06FF]"))
        {
            return "ar"; // Arabic
        }
        ...
        if (Regex.IsMatch(input, @"[\uAC00-\uD7AF]"))
        {
            return "ko"; // Hangul
        }

        if (Regex.IsMatch(input, @"[\u3040-\u309F\u30A0-\u30FF]"))
        {
            return "ja"; // Hiragana + Katakana
        }

        if (Regex.IsMatch(input, @"[\u4E00-\u9FFF]"))
        {
            return "zh"; // Han (CJK)
        }

        if (Regex.IsMatch(input, @"[\u0900-\u097F]"))
        {
        ...
        if (Regex.IsMatch(input, @"[\u0A80-\u0AFF]"))
        {
            return "gu"; // Gujarati
        }

        if (Regex.IsMatch(input, @"[\u0C80-\u0CFF]"))
        {
            return "kn"; // Kannada
        }
        ...
        if (Regex.IsMatch(input, @"[\u1200-\u137F]"))
        {
            return "am"; // Ethiopic
        }

        if (Regex.IsMatch(input, @"[\u0370-\u03FF]"))
        {
            return "el"; // Greek
        }

        // Latin-based languages with diacritics
        if (Regex.IsMatch(input, @"[à-ỹÀ-Ỹ]"))
        {
            return "vi"; // Vietnamese
        }

        if (Regex.IsMatch(input, @"[éèêëàâäîïôöùûüç]"))
        {
            return "fr"; // French
        }

        if (Regex.IsMatch(input, @"[ñáéíóúü¡¿]"))
        {
            return "es"; // Spanish
        }
        ...
        if (Regex.IsMatch(input, @"[åäö]"))
        {
            return "sv"; // Swedish
        }

        if (Regex.IsMatch(input, @"[æøå]"))
        {
            return "no"; // Norwegian
        }

        if (Regex.IsMatch(input, @"[ðþæö]"))
        {
            return "is"; // Icelandic
        }

        if (Regex.IsMatch(input, @"[çãõ]"))
        {
            return "pt"; // Portuguese
        }

        if (Regex.IsMatch(input, @"[čćžšđ]"))
        {
            return "hr"; // Croatian/Serbian
        }

        if (Regex.IsMatch(input, @"[șțăâî]"))
        {
            return "ro"; // Romanian
        }

        if (Regex.IsMatch(input, @"[łżźćń]"))
        {
            return "pl"; // Polish
        }

        if (Regex.IsMatch(input, @"[čšěřůž]"))
        {
            return "cs"; // Czech
        }

        if (Regex.IsMatch(input, @"[őű]"))
        {
            return "hu"; // Hungarian
        }

        if (Regex.IsMatch(input, @"[ğüşöçİ]"))
        {
            return "tr"; // Turkish
        }

        if (Regex.IsMatch(input, @"[õäöü]"))
        {
            return "et"; // Estonian
        }

        if (Regex.IsMatch(input, @"[ųėįū]"))
        {
            return "lt"; // Lithuanian
        }

        if (Regex.IsMatch(input, @"[āēīū]"))
        {
            return "lv"; // Latvian
        }

        if (Regex.IsMatch(input, @"[əğçşöü]"))
        {
            return "az"; // Azerbaijani
        }

        // Latin keyword-based heuristics
        if (Regex.IsMatch(input, @"\b(saya|anda|kamu|tidak|apa)\b", RegexOptions.IgnoreCase))
        {
            return "id"; // Indonesian
        }

        if (Regex.IsMatch(input, @"\b(ako|ikaw|sila|kayo|natin)\b", RegexOptions.IgnoreCase))
        {
            return "fil"; // Filipino
        }

        if (Regex.IsMatch(input, @"\b(malay|saya|tidak|apa)\b", RegexOptions.IgnoreCase))
        {
            return "ms"; // Malay
        }

        if (Regex.IsMatch(input, @"\b(eu|você|não|sim|como)\b", RegexOptions.IgnoreCase))
        {
            return "pt"; // Portuguese (fallback)
        }
