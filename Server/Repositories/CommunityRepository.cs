...
public class CommunityRepository : ICommunityRepository
{
    ...
    public async Task TransferModeratorAsync(int communityId, int currentModeratorId, int newModeratorId)
    {
        _logger.LogInformation(
            "Starting moderator transfer for community {CommunityId} from {CurrentModeratorId} to {NewModeratorId}",
        ...
        try
        {
            ...
            {
                _logger.LogWarning("User {UserId} attempted to transfer moderator role but is not a moderator",
                    currentModeratorId);
            ...
            {
                _logger.LogWarning("User {UserId} attempted to transfer moderator role to themselves",
                    currentModeratorId);
            ...
            _logger.LogInformation(
                "Successfully transferred moderator role for community {CommunityId} from {CurrentModeratorId} to {NewModeratorId}",
                communityId, currentModeratorId, newModeratorId);
        }
        catch (Exception ex)
        {
    ...
    public async Task<IEnumerable<CommunityDTO>> GetUserCommunitiesAsync(int userId)
    {
        ...
        try
        {
            ...
            foreach (var community in communities)
                ...
                {
                    _logger.LogInformation(
                        $"[DEBUG] Processing community: {community.Name} (ID: {community.CommunityID})");

                    var member = community.Members?.FirstOrDefault(m => m.UserID == userId);
                    if (member == null)
                    {
                        _logger.LogWarning(
                            $"[DEBUG] Could not find membership for user {userId} in community {community.CommunityID}");
                        continue;
                    }
                    ...
                    _logger.LogInformation(
                        $"[DEBUG] Created DTO for community: {dto.Name} with {dto.Members.Count} members");
                    result.Add(dto);
                }
                catch (Exception ex)
                {
