...
public class NotificationsController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<NotificationDTO>>> GetNotifications([FromQuery] bool unreadOnly = false)
    {
        try
        {
            _logger.LogInformation(
                $"[NotificationsController] Starting GetNotifications. User authenticated: {User.Identity.IsAuthenticated}");

            var userId = GetCurrentUserId();
            _logger.LogInformation(
                $"[NotificationsController] Getting notifications for user ID: {userId}, unreadOnly: {unreadOnly}");

            var notifications = await _notificationRepository.GetUserNotificationsAsync(userId, unreadOnly);
            _logger.LogInformation(
                $"[NotificationsController] Found {notifications.Count()} notifications for user {userId}");

            // Log the first few notifications for debugging
            foreach (var notification in notifications.Take(3))
                _logger.LogInformation(
                    $"[NotificationsController] Notification {notification.NotificationID} - Type: {notification.Type}, Status: {notification.Status}, Message: {notification.Message}");
    ...
    public async Task<ActionResult<IEnumerable<NotificationDTO>>> GetRecentNotifications()
    {
        try
        {
            ...
            _logger.LogInformation(
                $"[NotificationsController] Found {notifications.Count} recent notifications for user {userId}");
